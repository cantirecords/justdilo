...
export default function WidgetApp({ initialTasks }: { initialTasks: SlimTask[] }) {
  ...
  async function submit(blob: Blob, mime: string) {
    ...
    fd.append("audio", new File([blob], `rec.${ext}`, { type: blob.type }));
  ...
  const overdue = tasks.filter(t => t.due_date && new Date(t.due_date) < new Date(todayStr) && new Date(t.due_date).toDateString() !== todayStr);
        ...
        <div className="flex-1 overflow-y-auto px-2.5 pb-3 min-h-0">
          {overdue.length > 0 && (
            <Section label="⚠ Overdue" labelClass="text-red-500">
              {overdue.map(t => <TaskRow key={t.id} task={t} onComplete={complete} overdue />)}
            </Section>
          )}
          {todayTasks.length > 0 && (
            <Section label="Today">
              {todayTasks.map(t => <TaskRow key={t.id} task={t} onComplete={complete} />)}
            </Section>
          )}
          {noDate.length > 0 && (
            <Section label="Pending">
              {noDate.map(t => <TaskRow key={t.id} task={t} onComplete={complete} />)}
            </Section>
          )}
          {tasks.length === 0 && (
...
function TaskRow({ task, onComplete, overdue = false }: { task: SlimTask; onComplete: (id: string) => void; overdue?: boolean }) {
  ...
  return (
    <div className={cn(
      ...
      <span className={cn(
        ...
        overdue ? "text-red-600/90 dark:text-red-400" : "text-black/70 dark:text-white/70",
      )}>
        {task.title}
      </span>
